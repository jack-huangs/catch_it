...
import numpy as np
from envs.utils.arm_controller import JointCompliantController
from constants import ARM_RPC_HOST, ARM_RPC_PORT, RPC_AUTHKEY
from envs.utils.arm_ik_solver import IKSolver
from envs.utils.kinova import TorqueControlledArm
...
class Arm:
    ...
    def execute_action(self, action):
        ...
        elif 'arm_pos' in action and 'arm_quat' in action:
            if self.ik_solver is None:
                raise ValueError("IK solver not available, but IK action requested.")
            qpos = self.ik_solver.solve(action['arm_pos'], action['arm_quat'], self.arm.q)
        self.command_queue.put((qpos, action['gripper_pos'].item()))

    def get_qpos(self):
        qpos = self.arm.q
        qpos_wrapped = (qpos + np.pi) % (2 * np.pi) - np.pi
        return qpos_wrapped
