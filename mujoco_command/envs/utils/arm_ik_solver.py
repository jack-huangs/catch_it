...
class IKSolver:
    def __init__(self, ee_offset=0.0):
        ...
        self.model = mujoco.MjModel.from_xml_path('mj_assets/kinova_gen3/gen3.xml')
...
if __name__ == '__main__':
    ...
    retract_qpos = (retract_qpos) % (2 * np.pi)
    solved_qpos = ik_solver.solve(home_pos, home_quat, retract_qpos).round(2)
    solved_qpos = (solved_qpos) % (2 * np.pi)
    print(retract_qpos, solved_qpos)
