...
class Stopwatch:
    ...
    def summary(self, reset=True):
        headers = ["name", "num", "t/call (ms)", "max t (ms)", "%"]
        total = 0
        times = {}
        for k, v in self.times.items():
            ...
            mean_t = sum_t / len(v)
            max_t = np.max(v)
            times[k] = (len(v), sum_t, mean_t, max_t)
            total += sum_t

        print("Timer Info:")
        rows = []
        for k, (num, sum_t, mean_t, max_t) in times.items():
            rows.append([
                k,
                f"{num:.1f}",
                f"{mean_t:.1f}",
                f"{max_t:.1f}",
                f"{100 * sum_t / total:.1f}"
            ])

        rows.append([
            "total(s)",
            1,
            f"{total/len(self.times):.1f}",
            "-",
            f"{100:.1f}"
        ])
